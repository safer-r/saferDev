test_that("all_args_here()", {
  ...
  # sophisticated example
    ...
    testthat::expect_no_error(saferDev::all_args_here(
        x = saferDev::all_args_here, # R function
    ...
    expected3 <- base::data.frame(
        LINE_NB = base::c(2, 2, 2, 2, 2, 3, 3, 3, 3, 3, 4, 5, 5), 
        FUN_NAME = base::c("as.vector", "unlist", "mapply", "rep", "length", "as.vector", "unlist", "mapply", "rep", "length", "do.call", "do.call", "parent.frame"), 
        FUN_ARGS = base::c('as.vector(x = base::unlist(x = base::mapply(FUN = function(x, y){base::rep(x = y, base::length(x = x))}, x = x, y = y, MoreArgs = NULL, SIMPLIFY = TRUE, USE.NAMES = TRUE), recursive = TRUE, use.names = TRUE), mode = "any")', 'unlist(x = base::mapply(FUN = function(x, y){base::rep(x = y, base::length(x = x))}, x = x, y = y, MoreArgs = NULL, SIMPLIFY = TRUE, USE.NAMES = TRUE), recursive = TRUE, use.names = TRUE)', 'mapply(FUN = function(x, y){base::rep(x = y, base::length(x = x))}, x = x, y = y, MoreArgs = NULL, SIMPLIFY = TRUE, USE.NAMES = TRUE)', 'rep(x = y, base::length(x = x))', 'length(x = x)', 'as.vector(x = base::unlist(x = base::mapply(FUN = function(x, y){base::rep(x = y, base::length(x = x))}, x = x, y = y)))', 'unlist(x = base::mapply(FUN = function(x, y){base::rep(x = y, base::length(x = x))}, x = x, y = y))', 'mapply(FUN = function(x, y){base::rep(x = y, base::length(x = x))}, x = x, y = y)', 'rep(x = y, base::length(x = x))', 'length(x = x)', 'do.call(what = base::c, args = code_for_col)', 'do.call(what = base::c, args = code_for_col, quote = FALSE, envir = base::parent.frame())', 'parent.frame()'), 
        FUN_POS = base::c(23, 43, 60, 94, 111, 24, 44, 61, 95, 112, 25, 26, 100), 
        DEF_ARGS = base::c('x, mode = "any"', 'x, recursive = TRUE, use.names = TRUE', 'FUN, ..., MoreArgs = NULL, SIMPLIFY = TRUE, USE.NAMES = TRUE', 'x, ...', 'x', 'x, mode = "any"', 'x, recursive = TRUE, use.names = TRUE', 'FUN, ..., MoreArgs = NULL, SIMPLIFY = TRUE, USE.NAMES = TRUE', 'x, ...', 'x', 'what, args, quote = FALSE, envir = parent.frame()', 'what, args, quote = FALSE, envir = parent.frame()', 'n = 1'), 
        MISSING_ARG_NAMES = base::c('', '', '', '', '', 'mode', 'recursive, use.names', 'MoreArgs, SIMPLIFY, USE.NAMES', '', '', 'quote, envir', '', 'n'), 
        MISSING_ARGS = base::c('', '', '', '', '', 'mode = "any"', 'recursive = TRUE, use.names = TRUE', 'MoreArgs = NULL, SIMPLIFY = TRUE, USE.NAMES = TRUE', '', '', 'quote = FALSE, envir = parent.frame()', '', 'n = 1'), 
        STATUS = base::c(
                'GOOD', 
                'GOOD', 
                'GOOD', 
    ...
    result4 <- saferDev::all_args_here(
        x = FUN2, # R function
        export = FALSE, # export the data frame into a .tsv file?
        path_out = ".", # pathway of the folder where to export the data frame
        df_name = "a.tsv", # name of the exported data frame file
    ...
    expected4 <- base::data.frame(
        LINE_NB = base::c(2,2), 
        FUN_NAME = base::c("do.call", "parent.frame"), 
        FUN_ARGS = base::c("do.call(what = base::c, args = code_for_col, quote = FALSE, envir = base::parent.frame())", "parent.frame()"), 
        FUN_POS = base::c(26, 100), 
        DEF_ARGS = base::c("what, args, quote = FALSE, envir = parent.frame()", "n = 1"), 
        MISSING_ARG_NAMES = base::c("", "n"), 
        MISSING_ARGS = base::c("", "n = 1"), 
        STATUS = base::c("GOOD", "parent.frame(n = 1)")
    )
