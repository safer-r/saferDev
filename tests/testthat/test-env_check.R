test_that("env_check()", {
    ...
    fun1 <- function(){t.test <- 0 ; mean <- 5 ; saferDev::env_check(pos = 1)} 
    result3 <- fun1()
    ...
    fun2 <- function(){sum <- 0 ; saferDev::env_check(pos = 2)} 
    result4 <- fun2()
    expected4 <- NULL
    ...
    fun3 <- function(){t.test <- 0 ; mean <- 5 ; saferDev::env_check(pos = 1, name = "fun3")}
    result5 <- fun3()
    ...
    # Alternative way:
    fun4 <- function(){t.test <- 0 ; mean <- 5 ; saferDev::env_check(pos = 1, name = base::as.character(base::sys.calls()[[base::length(base::sys.calls())]]))}
    result6 <- fun4()
    ...
    fun7 <- function(){
        ...
        fun8 <- function(){
            ...
            range <- "a"
            saferDev::env_check(pos = test.pos, name = if(base::length(base::sys.calls()) >= test.pos){
