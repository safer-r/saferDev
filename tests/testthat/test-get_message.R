testthat::test_that("get_message()", {
    ...
    testthat::expect_error(get_message(data = char1, kind = "error", print.no = TRUE, text = "IN A", safer_check = 'TRUE'))
    testthat::expect_error(get_message( kind = "warning", print.no = TRUE, text = "IN A", safer_check = FALSE))
    testthat::expect_no_error(get_message(data = char1, kind = "message", print.no = TRUE,text = NULL, safer_check = FALSE))
    testthat::expect_error(get_message(data = char1, kind = NULL, print.no = TRUE,text = NULL, safer_check = FALSE))
    testthat::expect_no_error(get_message(data = "", kind = "error", print.no = TRUE,text = NULL, safer_check = FALSE)) # because data can be ""
    testthat::expect_no_error(get_message(data = NOT_CHARACTER, kind = "error", print.no = TRUE,text = NULL, safer_check = FALSE))
    ...
    expected6 <- "simpleWarning in base::eval(expr = base::parse(text = data, file = \"\", n = NULL, : ohoh\n"
    testthat::expect_equal(result6, expected6)

    result7 <- get_message(data = char4, kind = "error",print.no = TRUE, text = "IN A")
    expected7 <- "NO ERROR MESSAGE REPORTED IN A"
    testthat::expect_equal(result7,expected7)

    result8 <- get_message(data = char5, kind = "message",print.no = TRUE, text = "IN INSTRUCTION 1")
