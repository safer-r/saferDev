...
arg_test <- function(
    ...
    if(parall == TRUE){
        if( ! base::is.null(x = thread.nb)){
            tempo <- saferDev::arg_check(data = thread.nb, typeof = "integer", mode = NULL, double_as_integer_allowed = TRUE, neg_values = FALSE, length = 1, prop = FALSE, double_as_integer_allowed = TRUE, options = NULL, all_options_in_data = FALSE, na_contain = FALSE, neg_values = TRUE, inf_values = TRUE, print = FALSE, data_name = NULL, fun_name = function_name, pack_name = NULL, safer_check = FALSE) ; base::eval(expr = ee, envir = base::environment(fun = NULL), enclos = base::environment(fun = NULL))
