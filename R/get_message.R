...
#' }
#' 
#' The \code{env} argument is used as value for the \code{envir} argument of the \code{base::eval()} function.
#' 
...
get_message <- function(
    ...
    if( ! base::is.null(x = text)){
        tempo <- saferDev::arg_check(data = text, class = NULL, typeof = NULL, mode = "character", length = 1, prop = FALSE, double_as_integer_allowed = TRUE, options = NULL, all_options_in_data = FALSE, na_contain = TRUE, neg_values = TRUE, inf_values = TRUE, print = FALSE, data_name = NULL, data_arg = TRUE, safer_check = FALSE, lib_path = lib_path, error_text = embed_error_text) ; base::eval(expr = ee, envir = base::environment(fun = NULL), enclos = base::environment(fun = NULL))
    }
