...
#' @title .function_check
#' @description
#' Check if required functions are present in required packages.
#' @param req.function Character vector of function names to check, preceded by the package of the function. See examples.
#' @param lib.path Character vector specifying the absolute pathways of the directories containing some of the listed packages in the req.package argument, if not in the default directories. Ignored if NULL.
#' @param external.function.name Name of the function using the .function_check() function.
#' @returns Nothing.
#' @examples
#' # .function_check(req.function = "ggplot2::notgood") # commented because this example returns an error
#' .function_check(req.function = "ggplot2::geom_point")
#' @keywords internal
#' @rdname internal_function
...
.function_check <- function(
    ...
    # DEBUGGING
    # req.function = "ggplot2" ; lib.path = "C:/Program Files/R/R-4.3.1/library"
    # req.function = "serpentine" ; lib.path = "C:/Users/yhan/AppData/Local/Temp/RtmpkfYz9V/RLIBS_2cb051b94b38"
    # function name
    function.name <- paste0(as.list(match.call(expand.dots = FALSE))[[1]], "()")
    arg.names <- names(formals(fun = sys.function(sys.parent(n = 2)))) # names of all the arguments
    arg.user.setting <- as.list(match.call(expand.dots = FALSE))[-1] # list of the argument settings (excluding default values not provided by the user)
    ...
    tempo <- NULL
    for(i1 in 1:length(req.function)){
        split <- base::strsplit(req.function[i1], "::")[[1]]
        tempo.log <- base::exists(split[2], envir = base::asNamespace(split[1]))
        if( ! tempo.log){
            tempo <- c(tempo, req.function[i1])
        }
    }
    if( ! is.null(tempo)){
        tempo.cat <- paste0(
