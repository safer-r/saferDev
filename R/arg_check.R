...
#' @param prop Single logical value. Are the numeric values between 0 and 1 (proportion)? If \code{TRUE}, can be used alone, without considering class, etc.
#' @param double_as_integer_allowed Single logical value. If \code{TRUE}, no error is reported in the checking message if argument is set to \code{typeof = "integer"} or \code{class = "integer"}, while the reality is \code{typeof == "double"} or \code{class == "numeric"} but the numbers strictly have zero as modulo (reminder of a division). This means that \code{i <- 1}, which is \code{typeof(i) -> "double"} is considered as integer with \code{double_as_integer_allowed = TRUE}. WARNING: \code{data \%\% 1 == 0L} but not \code{isTRUE(all.equal(data \%\% 1, 0))} is used here because the argument checks for integers stored as double (does not check for decimal numbers that are approximate integers).
...
#' @param all_options_in_data Single logical value. If \code{TRUE}, all of the options must be present at least once in the \code{data} argument, and nothing else. If \code{FALSE}, some or all of the options must be present in the \code{data} argument, and nothing else. Ignored if \code{options} is \code{base::is.null}.
#' @param na_contain Single logical value. Can the \code{data} argument contain \code{NA}?
#' @param neg_values Single logical value. Are negative numeric values authorized? Warning: the default setting is \code{TRUE}, meaning that, in that case, no check is performed for the presence of negative values. The \code{neg_values} argument is activated only when set to \code{FALSE}. In addition, (1) \code{neg_values = FALSE} can only be used when \code{class}, \code{typeof} or \code{mode} arguments are not \code{NULL}, otherwise return an error message, (2) the presence of negative values is not checked with \code{neg_values = FALSE} if the tested object is a factor and the following checking message is returned \code{"OBJECT MUST BE MADE OF NON NEGATIVE VALUES BUT IS A FACTOR"}.
#' @param inf_values Single logical value. Are infinite numeric values authorized (\code{Inf} or \code{-Inf})? Identical remarks as for the \code{neg_values} argument.
#' @param print Single logical value. Print the message if \code{$problem} is \code{TRUE}? Warning: set by default to \code{FALSE}, which facilitates the control of the checking message output when using \code{arg_check()} inside functions. See the example section.
...
#' @param data_arg Single logical value. Is the tested object a function argument? If \code{TRUE} (default), \code{"ARGUMENT"} is written in messages, otherwise \code{"OBJECT"}.
